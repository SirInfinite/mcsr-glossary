import { MEDIA_CONTRACT, TERM_CONTRACT, isValidEditorialDate, slugifyTermName } from "./content-contract.js";
import { validateMediaItem } from "./content-contract.js";
import { analyzeMediaSlots } from "./content/media.js";
import { normalizeText as lowercase } from "./content/search.js";
...
export function validateGlossary(payload, { voteRowIDs = null } = {}) {
    ...
    if (!terms.length) errors.push("The published glossary must contain at least one term.");
    ...
    for (const [index, term] of terms.entries()) {
        ...
        if (typeof term.id !== "string" || !TERM_CONTRACT.uuidPattern.test(term.id)) {
            ...
            if (term.id !== idKey) errors.push(`${label}: UUID must use canonical lowercase hexadecimal.`);
        ...
        if (term.creationDate && term.updatedDate && term.updatedDate < term.creationDate) errors.push(`${label}: updatedDate cannot precede creationDate.`);
        if (Object.hasOwn(term, "legacySlugs")) {
            if (!Array.isArray(term.legacySlugs) || term.legacySlugs.length > 10
                || term.legacySlugs.some(value => typeof value !== "string" || value.length > 100 || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value))) {
                errors.push(`${label}: legacySlugs must contain at most ten lowercase URL slugs.`);
            }
        }
    ...
    // UUID, canonical slug and historical slugs share a single route namespace.
    const routeOwners = new Map();
    for (const term of terms) {
        if (!term || typeof term.name !== "string" || typeof term.id !== "string") continue;
        const routes = [term.id.toLowerCase(), slugifyTermName(term.name), ...(Array.isArray(term.legacySlugs) ? term.legacySlugs : [])];
        for (const route of routes) {
            if (typeof route !== "string") continue;
            if (routeOwners.has(route)) errors.push(`${term.name}: duplicate route '${route}' also used by ${routeOwners.get(route)}.`);
            else routeOwners.set(route, term.name);
        }
    }
