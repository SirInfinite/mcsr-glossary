// Run with Playwright CLI against the locally served GitHub Pages subpath:
...
// Service responses are fixtures: this suite never writes to a moderation queue.
// Run npm run test-live-voting separately for the real reversible RPC contract.
async page => {
    const base = 'http://127.0.0.1:8000/mcsr-glossary/';
    ...
    try {
        ...
        await tab.locator('#sub-submit').click();
        await tab.waitForFunction(() => document.querySelector('#sub-status').textContent.includes('has not been sent'));
        ...
        await context.route('**/js/supabase-config.js', async route => {
            const response = await route.fetch();
            await route.fulfill({ response, body: (await response.text()) + '\nwindow.MCSR_CONFIG = { ...window.MCSR_CONFIG, trendingEnabled: true };' });
        });
        await tab.reload();
        ...
        await tab.screenshot({ path: 'output/playwright/final/browser-test-failure.png' });
