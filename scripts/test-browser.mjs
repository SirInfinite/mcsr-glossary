...
import { releaseGates } from "../tests/browser/release-gates.mjs";
...
try {
    ...
    const release = await releaseGates(browser, base);
    const visual = await visualAudit(browser, base, { output });
    const result = { product, failures, media, release, visual };
